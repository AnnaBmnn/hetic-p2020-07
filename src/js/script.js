...
function loop()
{
    ...
    // Limite
    if ((coords.x > canvas.width - (80 * window.devicePixelRatio) && direct.ball1 === 1)
        || (coords.x < 0 && direct.ball1 === -1)) {
        direct.ball1 *= -1;
    }
    // Limite
    if ((coords.x_2 > canvas.width - (80 * window.devicePixelRatio) && direct.ball2 === 1) || (coords.x_2 < 0 && direct.ball2 === -1)) {
        direct.ball2 *= -1;
    }

    if ((coords.x_3 > canvas.width - (80 * window.devicePixelRatio)  && direct.ball3 === 1) || (coords.x_3 < 0 && direct.ball3 === -1)) {
        direct.ball3 *= -1;
    }

    // Dessin de la balle
    context.drawImage(image, coords.x, coords.y, 90 * window.devicePixelRatio, 80 * window.devicePixelRatio);
    context.drawImage(image, coords.x_2, coords.y_2, 90 * window.devicePixelRatio, 80 * window.devicePixelRatio);
    context.drawImage(image, coords.x_3, coords.y_3, 90 * window.devicePixelRatio, 80 * window.devicePixelRatio);
